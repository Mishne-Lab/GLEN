function [L,Y,offset,L_iter,Y_iter] = gl_gaussian_log(X_noisy,param)
...
% define initial Y
Y_0 = log(Y_0+1)-mean(log(Y_0+1),2);
% Y_0 = Y_0 - mean(Y_0,1);
Y = Y_0;
...
for i = 1:max_iter
    ...
    if (isfield(param, 'L_init') && i==1)
        ...
        switch reg_type
            ...
            case 'log'
                Z = gsp_distanz(Y').^2;
                W = gsp_learn_graph_log_degrees(Z/beta/2/T,1,gamma/beta/2,struct("maxit",200));
                W(W<0) = 0;
                L = diag(sum(W,1))-W;
            case 'cgl'
