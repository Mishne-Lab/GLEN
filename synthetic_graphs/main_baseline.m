...
% gaussian l2
beta_cgl = 10.^[0:-0.1:-1];
alpha_cgl = 10.^[-1:-0.2:-3];

% % gaussian log
% beta_cgl = 10.^[0.5:-0.1:-0.5];
% gamma_log = 10.^[-0:-0.1:-1];
% alpha_cgl = 10.^[-1:-0.2:-3];
...
%         param.lambda = beta_cgl(j);
        param.gamma = 0;%gamma_log(t);
%         [L,Y,offset,L_iter,O_iter] = vgl_poisson_log(X_noisy, param);
        [L,Y,offset,L_iter,Y_iter] = gl_gaussian_log(X_noisy, param);
%         Ycell_poiss_cgl1{i,j,t} = Y;
%         Ocell_poiss_cgl1{i,j} = O_iter;
        for k = 1:20
            if k <= size(L_iter,3)
                ...
                precision_poiss_cgl1(i,j,k) = precision_poiss_cgl1(i,j,t,size(L_iter,3));
                recall_poiss_cgl1(i,j,k) = recall_poiss_cgl1(i,j,t,size(L_iter,3));
                Fmeasure_poiss_cgl1(i,j,k) = Fmeasure_poiss_cgl1(i,j,t,size(L_iter,3));
                NMI_poiss_cgl1(i,j,k) = NMI_poiss_cgl1(i,j,t,size(L_iter,3));
                num_of_edges_poiss_cgl1(i,j,k) = num_of_edges_poiss_cgl1(i,j,t,size(L_iter,3));
                rel_err_poiss_cgl1(i,j,k) = rel_err_poiss_cgl1(i,j,t,size(L_iter,3));
