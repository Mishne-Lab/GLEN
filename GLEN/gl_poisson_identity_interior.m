function [L,Y,L_harvard,offset,L_iter] = gl_poisson_identity_interior(X_noisy,param)
...
%     break
    ...
    grad = - (Y_0+1/rho)./(Y+offset) + 1 + alpha/T * L * Y;
    hess = alpha/T * L + (eye(N).*repmat(reshape((Y_0+1/rho)./(Y+offset).^2,[1,N,T]),[N,1,1]));
...
%     x = fmincon(fun,x0,[],[],ones(1,N),zeros(N),zeros(N,T),[]);
