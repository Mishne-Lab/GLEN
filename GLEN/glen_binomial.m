function [L,Y,offset,L_iter,O_iter] = glen_binomial(X_noisy,param)
...
for i = 1:max_iter
    ...
    for j = 1:1
        grad = - Y_0 + K./(1+exp(-Y-offset)) + alpha/M * L * Y;
        hess = alpha/M * L + (eye(N).*repmat(reshape(K./(exp(Y+offset)+2+exp(-Y-offset)),[1,N,M]),[N,1,1]));
...
%         hess = eye(N).*repmat(reshape((1./V.^2 + exp(offset+Y+V/2)/2)/2, [1,N,M]),[N,1,1]);
%         D = -pagemldivide(hess,reshape(grad,[N,1,M]));
%         t = 0.5;
%         V = V+t*squeeze(D);
%         V(V<0) = 1e-4;
%     end
