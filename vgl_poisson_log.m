function [L,Y,offset,L_iter,O_iter,Y_iter] = vgl_poisson_log(X_noisy,param)
...
% mu = mean(X_noisy,2);
% sigma = cov(X_noisy');
% V = diag(log(((sigma-diag(mu))./mu)./mu'+1))*ones(1,T)/10;
...
%     sum(exp(offset+Y+exp(V)/2),'all') - sum(Y_0.*(offset+Y),'all') - sum(V, 'all')/2 + alpha/2*vec(Y*Y')'*vec(L)/T + alpha*beta*trace(L) - alpha/2*log(det(L+1/N))
    ...
    for j = 1:1
        grad = - Y_0 + exp(offset+Y+V/2) + alpha/T * L * Y;
        hess = alpha/T * L + (eye(N).*repmat(reshape(exp(offset+Y+V/2),[1,N,T]),[N,1,1]));
