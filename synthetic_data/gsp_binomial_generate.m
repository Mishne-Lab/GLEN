%% parameter set up
nreplicate = 4; % repeat the same experiment (based on different graphs)
num_of_signal = 2000;
N = 20;
K = 8;
gmodel = 'er';
...
while true
    switch gmodel
        case 'er'
            [A] = construct_graph(N,'er',0.3);
        case 'sbm'
            [A] = construct_graph(N,'sbm',0.4,0.1);
        case 'ws'
            [A] = construct_graph(N,'ws',2,0.1);
    end
    if all(conncomp(graph(A))==1)
        break;
    end
end
...
save("example_binomial_graph_signals.mat", "data", "K", "N", "num_of_signal", "nreplicate");
