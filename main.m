...
% % er offset = 2, -2
% beta_dong = 10.^[-1:-0.1:-2]; % design 1
% 
% % sbm offset = 2, -2
% beta_dong = 10.^[-0.5:-0.1:-1.5]; % design 1
% 
% % ws offset = 2, -2
% beta_dong = 10.^[-0.5:-0.1:-1.5]; % design 1
% 
...
% beta_dong = 10.^[-0.5:-0.1:-1.5]; % design 1
% alpha_dong = 10.^[1:-0.2:-1];
% 
% % er offset = 2, -2
% beta_dong = 10.^[-1.5:-0.1:-2.5]; % design 1
% % beta_cgl = 10.^[-1.5:-0.1:-2.5]; % design 1
% alpha_dong = 10.^[-1:-0.2:-3];
% 
% precision_poiss_dong = zeros(length(alpha_dong),length(beta_dong),20);
% recall_poiss_dong = zeros(length(alpha_dong),length(beta_dong),20);
% Fmeasure_poiss_dong = zeros(length(alpha_dong),length(beta_dong),20);
% NMI_poiss_dong = zeros(length(alpha_dong),length(beta_dong),20);
% num_of_edges_poiss_dong = zeros(length(alpha_dong),length(beta_dong),20);
% rel_err_poiss_dong = zeros(length(alpha_dong),length(beta_dong),20);
% 
% length_alpha = length(alpha_dong);
% length_beta = length(beta_dong);
% 
% % mu = mean(X_noisy,2);
% % sigma = cov(X_noisy');
...
%         param.reg_type = 'l2';
%         param.max_iter = 20;
%         param.alpha = alpha_dong(i);
%         param.beta = beta_dong(j);
%         param.gamma = 0;
% %         param.L_init = L;
% %         param.L_init = L_0;
% %         param.Y_init = X;
...
% % %         refinement
% %         S = (Y*Y')/T;
% %         L = estimate_cgl(S,double(L<10^(-4)),beta,1e-4,1e-6,40,2);
% %         L_iter(:,:,20) = L;
% 
%         Ycell_poiss_dong{i,j} = Y;
%         Ocell_poiss_dong{i,j} = O_iter;
%         for k = 1:20
%             if k <= size(L_iter,3)
%                 L = L_iter(:,:,k);
%                 Lcell_poiss_dong{i,j,k} = L;
%                 L(abs(L)<10^(-4))=0;
%                 if any(isnan(L))
%                     continue
%                 end
%                 [precision_poiss_dong(i,j,k),recall_poiss_dong(i,j,k),Fmeasure_poiss_dong(i,j,k),NMI_poiss_dong(i,j,k),num_of_edges_poiss_dong(i,j,k)] = graph_learning_perf_eval(L_0,L);
%                 rel_err_poiss_dong(i,j,k) = norm(L-L_0, 'fro') / norm(L_0, 'fro');
%             else
%                 precision_poiss_dong(i,j,k) = precision_poiss_dong(i,j,size(L_iter,3));
%                 recall_poiss_dong(i,j,k) = recall_poiss_dong(i,j,size(L_iter,3));
%                 Fmeasure_poiss_dong(i,j,k) = Fmeasure_poiss_dong(i,j,size(L_iter,3));
%                 NMI_poiss_dong(i,j,k) = NMI_poiss_dong(i,j,size(L_iter,3));
%                 num_of_edges_poiss_dong(i,j,k) = num_of_edges_poiss_dong(i,j,size(L_iter,3));
%                 rel_err_poiss_dong(i,j,k) = rel_err_poiss_dong(i,j,size(L_iter,3));
%             end
%         end
%     end
% end
% 
% result1_poiss_dong(:,:,:,ii) = precision_poiss_dong;
% result2_poiss_dong(:,:,:,ii) = recall_poiss_dong;
% result3_poiss_dong(:,:,:,ii) = Fmeasure_poiss_dong;
% result4_poiss_dong(:,:,:,ii) = NMI_poiss_dong;
% result5_poiss_dong(:,:,:,ii) = num_of_edges_poiss_dong;
% result6_poiss_dong(:,:,:,ii) = rel_err_poiss_dong;
% 
% graph_poiss_dong{ii} = Lcell_poiss_dong;
% offest_poiss_dong{ii} = Ocell_poiss_dong;
...
% er offset = 2, -2
beta_cgl = 10.^[-1:-0.1:-2]; % design 1
% beta_cgl = 10.^[-2.5:-0.1:-3.5]; % design 1
alpha_cgl = 10.^[-0.5:-0.1:-1.5];
% % vi5
...
% alpha_cgl = 10.^[0:-0.2:-2];
% alpha_cgl = 10.^[0:-0.1:-1];
% % beta_cgl = 10.^[-1:-0.1:-2]; %vi 5

% % ws offset = 2, -2
% beta_cgl = 10.^[-0.5:-0.1:-1.5]; % design 1 / %vi 5
% alpha_cgl = 10.^[0:-0.2:-2];
% alpha_cgl = 10.^[-0.1:-0.1:-1.1];
...
%         param.Y_init = X;
%         [L,Y,offset,L_iter] = gl_poisson_log(X_noisy, param);
        [L,Y,offset,L_iter,O_iter,Y_iter] = vgl_poisson_log(X_noisy, param);
...
%         Ycell_poiss_cgl1{i,j} = Y;
        ...
        for k = 1:20
            if k <= size(L_iter,3)
                ...
                rel_err_edge1_poiss_cgl1(i,j,k) = norm(edge-edge_0, 1) / norm(edge_0, 1);
                rel_err_edge2_poiss_cgl1(i,j,k) = norm(edge-edge_0, 2) / norm(edge_0, 2);
                rel_err_deg1_poiss_cgl1(i,j,k) = norm(deg-deg_0, 1) / norm(deg_0, 1);
                rel_err_deg2_poiss_cgl1(i,j,k) = norm(deg-deg_0, 2) / norm(deg_0, 2);
                rel_err_y_poiss_cgl1(i,j,k) = norm(X-Y_iter(:,:,k), 'fro') / norm(X, 'fro');
            else
                Lcell_poiss_cgl1{i,j,k} = L;
                ...
                rel_err_edge1_poiss_cgl1(i,j,k) = rel_err_edge1_poiss_cgl1(i,j,size(L_iter,3));
                rel_err_edge2_poiss_cgl1(i,j,k) = rel_err_edge2_poiss_cgl1(i,j,size(L_iter,3));
                rel_err_deg1_poiss_cgl1(i,j,k) = rel_err_deg1_poiss_cgl1(i,j,size(L_iter,3));
                rel_err_deg2_poiss_cgl1(i,j,k) = rel_err_deg2_poiss_cgl1(i,j,size(L_iter,3));
                rel_err_y_poiss_cgl1(i,j,k) = rel_err_y_poiss_cgl1(i,j,size(L_iter,3));
            end
        end
    end
end
...
result11_poiss_cgl1(:,:,:,ii) = rel_err_y_poiss_cgl1;
