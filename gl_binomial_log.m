function [L,Y,offset,L_iter,O_iter] = gl_binomial_log(X_noisy,param)
...
%     sum(exp(offset+Y+exp(V)/2),'all') - sum(Y_0.*(offset+Y),'all') - sum(V, 'all')/2 + alpha/2*vec(Y*Y')'*vec(L)/T + alpha*beta*trace(L) - alpha/2*log(det(L+1/N))
    ...
    for j = 1:1
        grad = - Y_0 + 2./(1+exp(-Y-offset)) + alpha/T * L * Y;
        hess = alpha/T * L + (eye(N).*repmat(reshape(2./(exp(Y+offset)+2+exp(-Y-offset)),[1,N,T]),[N,1,1]));
